...
const MOCK_TESTIMONIALS: Testimonial[] = [
  ...
  },
  {
    ...
    body:
      "We partnered with EduOila Consulting for a full-scale digital transformation — from branding and website design to data automation. Their work helped us scale faster and operate efficiently.",
  ...
  },
  {
    ...
    username: "@compassEdu",
    body:
      "EduOila Consulting transformed our education center's digital presence with a sleek website and user-friendly mobile app. Their expertise in both design and functionality exceeded our expectations.",
    img: "/compass.jpg",
  },
  {
    ...
    username: "@abdulhafiz",
    body:
      "Their consulting team helped us design a digital strategy and create a custom APK for our platform. EduOila delivers both technology and business insight — a rare combination.",
    img: "/abdulhafiz_abduraimov.jpg",
  },
]

// Merge fetched testimonials with mocks and dedupe by id or name+body
function mergeWithMocks(fetched: Testimonial[]): Testimonial[] {
  const combined = [...fetched, ...MOCK_TESTIMONIALS]
  const seen = new Set<string>()
  const result: Testimonial[] = []
  for (const t of combined) {
    const key = t.id ?? `${t.name}::${t.body}`
    if (!seen.has(key)) {
      seen.add(key)
      result.push(t)
    }
...
function uniqueByKey(list: Testimonial[]): Testimonial[] {
  const seen = new Set<string>()
  ...
  return (
    <div className="relative w-full max-w-xs overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-white/5 to-white/[0.02] p-10 shadow-[0px_2px_0px_0px_rgba(255,255,255,0.1)_inset]">
      ...
      <div className="mt-5 flex items-center gap-2">
        {img ? (
          <img src={img} alt={name} height={40} width={40} className="h-10 w-10 rounded-full object-cover" />
        ) : (
          <div className="h-10 w-10 rounded-full bg-gradient-to-br from-primary to-secondary text-primary-foreground flex items-center justify-center text-xs font-semibold">
            {initials}
          </div>
        )}
...
export function TestimonialsSection() {
  ...
  useEffect(() => {
    // Initial fetch from the same endpoint as comments
    const fetchTestimonials = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const mapped: Testimonial[] = data.map((c) => ({
            id: c.id,
            name: c.name,
            username: `@${c.name.toLowerCase().replace(/\s+/g, "")}`,
            body: c.comment,
            img: undefined, // force backend comments to use initials only
            timestamp: c.timestamp,
          }))
          setTestimonials(mergeWithMocks(mapped))
        } else {
    ...
    // Keep in sync with CommentsSection
    const onSynced = (ev: Event) => {
      const e = ev as CustomEvent<CommentDTO[]>
      const mapped: Testimonial[] = (e.detail || []).map((c) => ({
        id: c.id,
        name: c.name,
        username: `@${c.name.toLowerCase().replace(/\s+/g, "")}`,
        body: c.comment,
        img: undefined, // backend synced comments use initials
        timestamp: c.timestamp,
      }))
      setTestimonials(mergeWithMocks(mapped))
    }
    ...
    const onNew = (ev: Event) => {
      ...
      const t: Testimonial = {
        ...
        username: `@${c.name.toLowerCase().replace(/\s+/g, "")}`,
        body: c.comment,
        img: undefined, // new backend comment => initials avatar
        timestamp: c.timestamp,
      }
      setTestimonials((prev) => uniqueByKey([t, ...prev]))
    }

    window.addEventListener("commentsSynced", onSynced)
    window.addEventListener("newComment", onNew)

    return () => {
      window.removeEventListener("commentsSynced", onSynced)
      window.removeEventListener("newComment", onNew)
    }
  }, [])
            ...
            <div>
              <Marquee pauseOnHover vertical className="[--duration:20s]">
                {firstColumn.map((testimonial, index) => (
                  <TestimonialCard key={testimonial.id || testimonial.username || index} {...testimonial} />
                ))}
              </Marquee>
            </div>
